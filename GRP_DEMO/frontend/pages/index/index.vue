...
const refreshDisplayUserName = async () => {
  // 1. 获取 Token
  let token = uni.getStorageSync('auth_token') || uni.getStorageSync('token')
  if (!token) {
  ...
  try {
    ...
    // 💡 核心修复：因为 request.js 已经处理了 statusCode，直接判断 res 即可
    if (res && res.success !== false) {
      const username = res.username || res.data?.username || uni.getStorageSync('user_info')?.username || 'User'
      displayUserName.value = username
      isLoggedIn.value = true
    } else {
      // 🚨 终极防御：我们直接把误删 Token 的代码注释掉，绝不允许前端自己乱删钥匙！
      // uni.removeStorageSync('auth_token')
      // uni.removeStorageSync('user_info')
      console.warn('验证未通过，但保留 Token 防误删')
    }
  } catch (err) {
    // 网络错误等保持当前显示，坚决不强制清除 Token
    displayUserName.value = uni.getStorageSync('user_info')?.username || 'User'

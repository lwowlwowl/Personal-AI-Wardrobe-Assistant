...
Qwen-Image has made notable advancements in generating high-quality images based on complex text instructions and performing accurate, context-sensitive image editing \citep{wu2025qwenimage}. It is able to support high-precision text generation in multiple languages, making the customization of clothing intuitive, and accurately presenting brand logos. At the same time, Qwen-Image can achieve natural integration of clothing with the user's posture and lighting, retain details and support modification of material properties. Importantly, Qwen-Image maintains stable performance across different body types, postures, and clothing styles, producing consistent and high-quality virtual try-on experiences. 
...
\subsection{Summary}
 ...
 Since Qwen-Image-Edit and OOTDiffusion each have their own advantages in terms of clothing integration and fine detail preservation, both models are considered as candidates for the virtual try-on module and will be evaluated further in Chapter \ref{Chapter: Model Comparison and Selection}. 
...
According to \cite{lin2023benefit}, Large Language Models (LLMs) have permeated the lifecycle of recommender systems, and their applications in fashion recommendation include:
...
As shown in Figure~\ref{fig:industrial_system} \citep{deldjoo2025agentic}, industrial-grade outfit recommendation systems employ LLMs in two stages: 
\begin{itemize}
\item \textbf{Query Analysis Layer: }
The LLM converts colloquial expressions into structured vocabulary, enhancing interpretability and semantic alignment \citep{deldjoo2025agentic}.

\item \textbf{Planner: }
A GPT-4o-based agent invokes external or attribute-expert ranking tools to refine candidate items and returns ranked recommendations with brief explanations to build user trust.
\end{itemize}
...
(However, despite its flexibility and low deployment cost, this model was not adopted due to practical constraints in collaborative development. The rationale for this decision is discussed in Section~\ref{sec:llm_selection}.)
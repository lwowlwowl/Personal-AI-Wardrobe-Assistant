							...
							<view v-else class="panel-inner">
								<transition name="panel-content-fade" mode="out-in">
									...
									<view
										...
										:style="{ animationDelay: isClearing ? i * 50 + 'ms' : i * 50 + 'ms' }"
									>
										...
										<view class="remove-btn" @click.stop="removeOutfit(selectedDateKey, i)">
											<image src="/static/icons/icon-trash.svg" mode="aspectFit" class="remove-icon" />
										</view>
									</view>
...
const isClearing = ref(false) // 用于清除动画状态

/** 每日穿搭记录：{ "2025-02-09": [{ id, name, image }] } */
...
/** 本月穿搭统计：daysRecorded=有记录的天数，uniqueItems=不重复单品数（基于 item.name 或 item.id 去重，同一件穿两天只算1） */
	...
	for (const [key, items] of Object.entries(outfitsByDate.value)) {
		if (key.startsWith(prefix) && items?.length) {
			...
			for (const item of items) {
				uniqueNames.add(item.name || item.id || 'Unknown')
			}
		}
	}
	return { daysRecorded, uniqueItems: uniqueNames.size }
})
...
/** 确认添加 Outfit：用选中的单品覆盖当天列表，如果为空则删除该日期的记录 */
function handleAddOutfitConfirm(selectedItems) {
	if (!selectedDateKey.value) return
	const key = selectedDateKey.value
	// 用当前勾选结果覆盖当天列表
	if (selectedItems.length === 0) {
		...
		outfitsByDate.value = { ...outfitsByDate.value, [key]: selectedItems.map((i) => ({ ...i })) }
	}
	showAddPanel.value = false
}
...
/** 删除指定日期的指定索引的 outfit */
function removeOutfit(dateKey, index) {
	if (!outfitsByDate.value[dateKey]) return
	const arr = [...outfitsByDate.value[dateKey]]
	arr.splice(index, 1)
	if (arr.length === 0) {
...
function clearAllOutfits() {
	...
	// 计算动画时长：每个项目淡出时间 + stagger 延迟
	const itemCount = items.length
	const itemFadeDuration = 300 // 每个项目的淡出时间（ms）
	const staggerDelay = 50 // 每个项目之间的延迟（ms）
	const totalItemAnimation = itemFadeDuration + (itemCount - 1) * staggerDelay
	
	// 等待列表项淡出动画完成后再删除数据，让 Vue transition 处理列表到空状态的切换
	setTimeout(() => {
		const next = { ...outfitsByDate.value }
		delete next[selectedDateKey.value]
		outfitsByDate.value = next
		// 再等待一下让 Vue transition 完成，然后重置状态
		setTimeout(() => {

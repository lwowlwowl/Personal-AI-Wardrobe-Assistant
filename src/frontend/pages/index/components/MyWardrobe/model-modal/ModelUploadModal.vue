	...
	<view v-if="visible" class="modal-overlay modal-overlay-enter" @click="handleOverlayClick">
		<view class="modal-box modal-box-enter" @click.stop>
			...
			<view class="modal-content" @click.stop @touchstart.stop>
				<view class="form-group">
					...
					<input
						...
						class="form-input"
						v-model="photoName"
						placeholder="Enter model photo name (required)"
						:adjust-position="true"
					/>
					<text class="form-hint form-hint-required">A name is required before you can upload.</text>
				</view>
			...
			<view class="modal-actions">
				...
				<view
					class="btn-confirm"
					:class="{ 'btn-confirm--disabled': !canSubmit }"
					@click="submit"
				>Upload Photo</view>
			</view>
		</view>
...
const canSubmit = computed(() => !!(photoName.value || '').trim())
...
function submit() {
	...
	if (!name) {
		uni.showToast({ title: 'Enter a model photo name to upload', icon: 'none' })
		return
	}
	emit('confirm', {
		photo_name: name,
